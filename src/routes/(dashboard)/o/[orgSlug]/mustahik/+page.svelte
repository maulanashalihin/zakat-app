...
<div class="p-4 lg:p-6" style="background-color: var(--bg-primary); min-height: 100%;">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 lg:mb-6">
    <div>
      <h1 class="text-xl lg:text-2xl font-bold" style="color: var(--text-primary);">Data Mustahik</h1>
      <p class="text-sm lg:text-base" style="color: var(--text-secondary);">Penerima zakat fitrah</p>
    </div>
    {#if user.role === 'admin' || user.role === 'super_admin'}
      <a href="/o/{organization.slug}/mustahik/tambah" class="btn-primary text-sm lg:text-base px-3 py-2 lg:px-4 lg:py-2 self-start">
      ...
      <table class="w-full min-w-[700px] lg:min-w-0">
        <thead>
          <tr style="background-color: var(--bg-tertiary);">
            ...
            <th class="px-2 lg:px-4 py-3 text-left text-xs lg:text-sm font-medium" style="color: var(--text-secondary);">Alamat</th>
            <th class="px-2 lg:px-4 py-3 text-left text-xs lg:text-sm font-medium" style="color: var(--text-secondary);">Status</th>
        ...
        <tbody class="divide-y" style="border-color: var(--border-primary);">
          {#each mustahik as m, i}
            <tr style="background-color: var(--bg-secondary);">
              ...
              <td class="px-2 lg:px-4 py-3 text-xs lg:text-sm truncate max-w-[150px] lg:max-w-none" style="color: var(--text-tertiary);">{m.alamat || '-'}</td>
              <td class="px-2 lg:px-4 py-3 text-xs lg:text-sm">
                <span class="inline-flex px-1.5 lg:px-2 py-0.5 lg:py-1 rounded-full text-xs font-medium"
                  style="background-color: {m.status === 'aktif' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(156, 163, 175, 0.1)'}; color: {m.status === 'aktif' ? '#22c55e' : '#9ca3af'};">
              ...
              <td class="px-2 lg:px-4 py-3">
                <div class="flex items-center gap-1 lg:gap-2">
                  <a href="/o/{organization.slug}/mustahik/{m.id}/edit" class="p-1.5 lg:p-2 rounded-lg hover:opacity-80" style="color: #3b82f6;" title="Edit">
          ...
          {:else}
            <tr style="background-color: var(--bg-secondary);">
              <td colspan="7" class="px-4 py-8 text-center text-sm" style="color: var(--text-tertiary);">
